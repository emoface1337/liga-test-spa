...
// 2. так как предоставленное API использует json-server под капотом  - значит, можно получить пользователей и сделать "?_embed=posts" в запросе,
//    получив список пользователей со свойстом постов для каждого пользователя { ...user, posts: [] }
